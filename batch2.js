...
async function batchMsisdnToIccidLookup(msisdnList) {
  ...
  // =========================================
  // ✅ NEW: ICCID-ONLY LIST (EXCLUDES NOT FOUND)
  // =========================================
  const iccidOnlyList = formattedResults
    .filter(r => r.iccid.trim() !== "") // Exclude empty/missing ICCIDs
    .map(r => r.iccid)
    .join("\n");
  ...
  // ===== ALIGNED VIEW (EASY COPY BLOCK) - TOP SECTION =====
  const alignedSection = document.createElement('div');
  alignedSection.style.cssText = 'margin-bottom: 25px;';
  ...
  // ===== ICCID-ONLY LIST SECTION =====
  const iccidOnlySection = document.createElement('div');
  iccidOnlySection.style.cssText = 'margin: 25px 0; padding-top: 15px; border-top: 1px solid #eee;';
  
  const iccidOnlyTitle = document.createElement('h3');
  iccidOnlyTitle.textContent = '📱 ICCID-Only List (Excludes Not Found):';
  ...
  iccidOnlyPre.textContent = iccidOnlyList || "(No valid ICCIDs found)";
  ...
  copyIccidOnlyBtn.onclick = async () => {
    if (!iccidOnlyList.trim()) {
      showFeedback('⚠️ No ICCIDs to copy', '#fff3cd', '#856404');
      return;
    }
    try {
  ...
  // Download button (now downloads aligned view)
  ...
  downloadBtn.onclick = () => {
    // Download combined output (aligned view + ICCID list)
    const fullOutput = 
      "✨ ALIGNED VIEW\n" + 
      alignedOutput + 
      "\n\n📱 ICCID-ONLY LIST (Excludes Not Found)\n" + 
      (iccidOnlyList || "(No valid ICCIDs found)");
